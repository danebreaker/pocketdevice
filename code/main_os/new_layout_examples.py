...
def main():
    ...
    #date
    display.draw_image('zero_tiny_dark.raw', 160, 1, 10, 14)
    display.draw_image('three_tiny_dark.raw', 170, 1, 10, 14)
    
    display.draw_image('slash_dark.raw', 180, 1, 10, 14)
    
    display.draw_image('zero_tiny_dark.raw', 190, 1, 10, 14)
    display.draw_image('nine_tiny_dark.raw', 200, 1, 10, 14)
    
    display.draw_image('slash_dark.raw', 210, 1, 10, 14)
    
    display.draw_image('two_tiny_dark.raw', 220, 1, 10, 14)
    display.draw_image('five_tiny_dark.raw', 230, 1, 10, 14)
    
    #day of week
    #display.draw_text(58, 65, 'Sunday', UbuntuMono, color565(122, 60, 31), background=color565(52, 51, 50))
    display.draw_text(55, 75, 'Sunday', arcadepix, color565(122, 60, 31), background=color565(52, 51, 50))
    
    #time
    display.draw_image('three.raw', 45, 90, 45, 65)
    display.draw_image('two.raw', 95, 90, 45, 65)
    display.draw_image('three.raw', 45, 160, 45, 65)
    display.draw_image('zero.raw', 95, 160, 45, 65)
    
    #seconds
    display.draw_image('zero_tiny.raw', 50, 230, 10, 14)
    display.draw_image('zero_tiny.raw', 65, 230, 10, 14)
    
    #am/pm
    display.draw_text(110, 225, 'AM', UbuntuMono, color565(122, 60, 31), background=color565(52, 51, 50))
